...
const NAV_ITEMS = [
  ...
  { id: "seasonality", icon: "📅", label: "Seasonality", minStage: 2, section: "Tools" },
...
export default function Sidebar({ activePage, onNavigate }) {
  ...
  const sections = {};
  for (const item of visible) { if (!sections[item.section]) sections[item.section] = []; sections[item.section].push(item); }

  return (
    <aside style={{ width: 220, minWidth: 220, height: "100%", background: "var(--bg-secondary)", borderRight: "1px solid var(--border)", padding: "16px 12px", overflowY: "auto" }}>
      <div style={{ margin: "0 8px 16px", padding: "10px 12px", borderRadius: 10, background: meta.bg, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div><div style={{ fontSize: 12, fontWeight: 700, color: meta.color }}>{meta.label}</div><div style={{ fontSize: 10, color: "var(--text-muted)" }}>Stage {stage} · {visible.length} features</div></div>
      </div>
      {Object.entries(sections).map(([sectionName, items]) => (
        <div key={sectionName} style={{ marginBottom: 20 }}>
          ...
          {items.map(item => {
            ...
            return (
              <div key={item.id} onClick={() => onNavigate(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 8, fontSize: 14, color: isActive ? "var(--green)" : "var(--text-secondary)", cursor: "pointer", transition: "all 0.15s", background: isActive ? "var(--green-dim)" : "transparent", fontWeight: isActive ? 600 : 400 }}
      ...
      {hidden.length > 0 && (
        <div style={{ margin: "8px 8px 16px" }}>
          <button onClick={() => setShowMore(!showMore)} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, background: showMore ? "var(--blue-dim)" : "var(--surface)", border: `1px solid ${showMore ? "rgba(96,165,250,0.2)" : "var(--border)"}`, color: showMore ? "var(--blue)" : "var(--text-muted)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ...
          {showMore && (<div style={{ marginTop: 8 }}>{hidden.map(item => (
            <div key={item.id} onClick={() => onNavigate(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, fontSize: 13, color: "var(--text-muted)", cursor: "pointer", opacity: 0.6, transition: "all 0.15s" }}
              ...
              <span style={{ width: 20, textAlign: "center", fontSize: 14 }}>{item.icon}</span>{item.label}
      ...
      <div style={{ margin: "8px 8px 0", padding: 12, background: "var(--bg-card)", borderRadius: 10, border: "1px solid var(--border)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><span style={{ fontSize: 12, color: "var(--text-secondary)" }}>🔥 Streak</span><span style={{ fontSize: 14, fontWeight: 700, color: "var(--amber)" }}>{userProfile?.streak_current || 0} days</span></div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: 12, color: "var(--text-secondary)" }}>💰 Portfolio</span><span style={{ fontSize: 14, fontWeight: 700, color: "var(--green)" }}>₹{((userProfile?.virtual_portfolio_value || 100000) / 100000).toFixed(1)}L</span></div>
      </div>
    </aside>
  );
}
