...
export default function TradingChart({ symbol, ltp, changePercent }) {
  ...
  useEffect(() => {
    ...
    activeIndicators.forEach(indId => {
      try {
        ...
        if (!ind) return;
        if (ind.type === 'overlay') {
          if (indId === 'sma20') { const d = calcSMA(chartData, 20); const s = chart.addLineSeries({ color: ind.color, lineWidth: 1.5 }); s.setData(d); indicatorSeriesRef.current.push(s); }
          ...
          else if (indId === 'bb') { const { upper, middle, lower } = calcBollingerBands(chartData); const su = chart.addLineSeries({ color: ind.color, lineWidth: 1, lineStyle: 2 }); su.setData(upper); indicatorSeriesRef.current.push(su); const sm = chart.addLineSeries({ color: ind.color, lineWidth: 1.5 }); sm.setData(middle); indicatorSeriesRef.current.push(sm); const sl = chart.addLineSeries({ color: ind.color, lineWidth: 1, lineStyle: 2 }); sl.setData(lower); indicatorSeriesRef.current.push(sl); }
        } else {
          if (indId === 'rsi') { const d = calcRSI(chartData); const s = chart.addLineSeries({ color: ind.color, lineWidth: 1.5, priceScaleId: 'rsi', lastValueVisible: true }); chart.priceScale('rsi').applyOptions({ scaleMargins: { top: 0.75, bottom: 0.02 } }); s.setData(d); indicatorSeriesRef.current.push(s); }
          else if (indId === 'macd') { const { macdLine, signalLine, histogram } = calcMACD(chartData); const sh = chart.addHistogramSeries({ priceScaleId: 'macd', priceFormat: { type: 'price', precision: 2, minMove: 0.01 } }); chart.priceScale('macd').applyOptions({ scaleMargins: { top: 0.78, bottom: 0.02 } }); sh.setData(histogram); indicatorSeriesRef.current.push(sh); const sm = chart.addLineSeries({ color: '#60a5fa', lineWidth: 1.5, priceScaleId: 'macd' }); sm.setData(macdLine); indicatorSeriesRef.current.push(sm); const ss = chart.addLineSeries({ color: '#f97316', lineWidth: 1.5, priceScaleId: 'macd' }); ss.setData(signalLine); indicatorSeriesRef.current.push(ss); }
          else if (indId === 'vdelta') { const d = calcVolumeDelta(chartData); const s = chart.addHistogramSeries({ priceScaleId: 'vdelta', priceFormat: { type: 'volume' } }); chart.priceScale('vdelta').applyOptions({ scaleMargins: { top: 0.78, bottom: 0.02 } }); s.setData(d); indicatorSeriesRef.current.push(s); }
          else if (indId === 'cvd') { const d = calcCVD(chartData); const s = chart.addLineSeries({ color: ind.color, lineWidth: 1.5, priceScaleId: 'cvd', lastValueVisible: true }); chart.priceScale('cvd').applyOptions({ scaleMargins: { top: 0.78, bottom: 0.02 } }); s.setData(d); indicatorSeriesRef.current.push(s); }
        }
  ...
  function toggleIndicator(id) { setActiveIndicators(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]); }
  const groups = {};
    ...
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px', borderBottom: '1px solid var(--border)', flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div><div style={{ fontSize: 16, fontWeight: 700 }}>{symbol}</div><div style={{ fontSize: 11, color: 'var(--text-muted)' }}>NSE · IST</div></div>
          <div><span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 22, fontWeight: 600 }}>₹{ltp?.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span><span style={{ fontSize: 13, fontWeight: 600, marginLeft: 8, color: changePercent >= 0 ? 'var(--green)' : 'var(--red)' }}>{changePercent >= 0 ? '+' : ''}{changePercent?.toFixed(2)}%</span></div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 3, background: 'var(--bg-secondary)', borderRadius: 8, padding: 3 }}>
            {[{ key: 'candlestick', label: '🕯️' }, { key: 'line', label: '📈' }].map(ct => (<button key={ct.key} onClick={() => setChartType(ct.key)} style={{ padding: '5px 10px', borderRadius: 6, fontSize: 14, border: 'none', cursor: 'pointer', background: chartType === ct.key ? 'var(--green-dim)' : 'transparent', color: chartType === ct.key ? 'var(--green)' : 'var(--text-muted)' }}>{ct.label}</button>))}
          </div>
          <div style={{ display: 'flex', gap: 2 }}>
            {TIMEFRAMES.map(tf => (<button key={tf.key} onClick={() => setActiveTimeframe(tf.key)} style={{ padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", background: activeTimeframe === tf.key ? 'var(--green-dim)' : 'transparent', border: `1px solid ${activeTimeframe === tf.key ? 'rgba(99,220,160,0.2)' : 'transparent'}`, color: activeTimeframe === tf.key ? 'var(--green)' : 'var(--text-muted)' }}>{tf.label}</button>))}
          </div>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowIndicatorPanel(!showIndicatorPanel)} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, border: '1px solid var(--border)', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", background: activeIndicators.length > 0 ? 'var(--blue-dim)' : 'transparent', color: activeIndicators.length > 0 ? 'var(--blue)' : 'var(--text-muted)' }}>📊 {activeIndicators.length > 0 ? `(${activeIndicators.length})` : 'Indicators'}</button>
            {showIndicatorPanel && (<div style={{ position: 'absolute', top: '120%', right: 0, zIndex: 50, width: 260, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, boxShadow: 'var(--shadow-lg)', padding: 8, maxHeight: 360, overflowY: 'auto' }}><div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', padding: '4px 8px', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>Indicators</div>{Object.entries(groups).map(([gn, items]) => (<div key={gn}><div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', padding: '8px 8px 4px', borderTop: '1px solid var(--border)', marginTop: 4 }}>{gn}</div>{items.map(ind => { const active = activeIndicators.includes(ind.id); return (<div key={ind.id} onClick={() => toggleIndicator(ind.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', borderRadius: 6, cursor: 'pointer', background: active ? 'var(--green-dim)' : 'transparent' }} onMouseEnter={e => { if (!active) e.currentTarget.style.background = 'var(--hover-subtle)'; }} onMouseLeave={e => { if (!active) e.currentTarget.style.background = 'transparent'; }}><span style={{ width: 10, height: 10, borderRadius: 3, background: ind.color, flexShrink: 0 }} /><span style={{ fontSize: 12, fontWeight: active ? 600 : 400, color: active ? 'var(--green)' : 'var(--text-secondary)', flex: 1 }}>{ind.name}</span><span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{ind.type === 'overlay' ? 'overlay' : 'sub'}</span></div>); })}</div>))}{activeIndicators.length > 0 && (<button onClick={() => setActiveIndicators([])} style={{ width: '100%', marginTop: 8, padding: '6px', borderRadius: 6, background: 'var(--red-dim)', color: 'var(--red)', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>Clear All</button>)}</div>)}
          </div>
        </div>
      </div>
      <div style={{ position: 'relative', height: activeIndicators.some(i => AVAILABLE_INDICATORS.find(a => a.id === i)?.type === 'subchart') ? 500 : 400, padding: '0 4px 4px' }}>
        {loading && (<div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isDark ? 'rgba(10,14,23,0.6)' : 'rgba(243,242,247,0.6)', zIndex: 10, borderRadius: 10, backdropFilter: 'blur(2px)' }}><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><div style={{ width: 18, height: 18, border: '2px solid var(--border)', borderTop: '2px solid var(--green)', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /><span style={{ color: 'var(--text-muted)', fontSize: 13 }}>Loading chart...</span></div></div>)}
