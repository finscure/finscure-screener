...
export default function TradingPage() {
  ...
  return (
    <div>
      {/* Portfolio Bar */}
      <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 18px", marginBottom: 18, display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap" }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: 22 }}>
          ...
          <div style={{ marginBottom: 16, position: "relative" }}>
            ...
            <input type="text" value={search || selectedStock} placeholder="Search stock..."
              ...
              style={{ width: "100%", padding: "10px 14px", background: "var(--input-bg)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-primary)", fontSize: 15, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600, outline: "none" }}
            />
            {showResults && searchResults.length > 0 && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, boxShadow: "var(--shadow-lg)", zIndex: 50, marginTop: 4, maxHeight: 200, overflowY: "auto" }}>
                {searchResults.map(s => (
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 18, background: "var(--bg-secondary)", borderRadius: 8, padding: 3 }}>
            <button onClick={() => { setOrderType("buy"); setOrderResult(null); }} style={{ flex: 1, padding: 9, border: "none", borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: "pointer", background: orderType === "buy" ? "var(--green-dim)" : "transparent", color: orderType === "buy" ? "var(--green)" : "var(--text-secondary)", fontFamily: "'DM Sans',sans-serif" }}>Buy</button>
            <button onClick={() => { setOrderType("sell"); setOrderResult(null); }} style={{ flex: 1, padding: 9, border: "none", borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: "pointer", background: orderType === "sell" ? "var(--red-dim)" : "transparent", color: orderType === "sell" ? "var(--red)" : "var(--text-secondary)", fontFamily: "'DM Sans',sans-serif" }}>Sell</button>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="number" value={quantity} onChange={e => { setQuantity(e.target.value); setOrderResult(null); }} placeholder="Shares"
              style={{ width: "100%", padding: "10px 14px", background: "var(--input-bg)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-primary)", fontSize: 15, fontFamily: "'JetBrains Mono',monospace", outline: "none" }}
            />
          </div>
          ...
          {parseInt(quantity) > 0 && stock.ltp > 0 && (
            <div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: 14, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 0" }}><span style={{ color: "var(--text-muted)" }}>Price</span><span style={{ fontWeight: 600, fontFamily: "'JetBrains Mono',monospace" }}>₹{stock.ltp?.toFixed(2)}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 0" }}><span style={{ color: "var(--text-muted)" }}>Qty</span><span style={{ fontWeight: 600, fontFamily: "'JetBrains Mono',monospace" }}>{quantity}</span></div>
              <div style={{ height: 1, background: "var(--border)", margin: "6px 0" }} />
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 700 }}><span>Total</span><span style={{ fontFamily: "'JetBrains Mono',monospace", color: orderType === "buy" ? "var(--green)" : "var(--red)" }}>₹{(parseInt(quantity) * stock.ltp).toLocaleString("en-IN", { maximumFractionDigits: 2 })}</span></div>
            </div>
          )}

          {orderResult && (<div style={{ padding: "10px 14px", borderRadius: 8, fontSize: 13, fontWeight: 600, marginBottom: 12, background: orderResult.ok ? "var(--green-dim)" : "var(--red-dim)", color: orderResult.ok ? "var(--green)" : "var(--red)" }}>{orderResult.ok ? "✓ " : "⚠ "}{orderResult.msg}</div>)}

          <button onClick={executeTrade} disabled={!parseInt(quantity) || !stock.ltp}
            style={{ width: "100%", padding: 13, border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: parseInt(quantity) && stock.ltp ? "pointer" : "default", background: orderType === "buy" ? "var(--gradient-green)" : "linear-gradient(135deg, #f87171, #ef4444)", color: orderType === "buy" ? "var(--btn-text)" : "#fff", fontFamily: "'DM Sans',sans-serif", opacity: parseInt(quantity) && stock.ltp ? 1 : 0.4 }}>
                  ...
                  {portfolio.holdings.map(h => {
                    ...
                    return (
                      <tr key={h.symbol}>
                        ...
                        <td>
                          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, color: pnl >= 0 ? "var(--green)" : "var(--red)" }}>{pnl >= 0 ? "+" : ""}{fmt(pnl)}</span>
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {transactions.map(tx => (
                    <tr key={tx.id}>
                      <td><span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 6, background: tx.type === "BUY" ? "var(--green-dim)" : "var(--red-dim)", color: tx.type === "BUY" ? "var(--green)" : "var(--red)" }}>{tx.type}</span></td>
