...
export default function TradingPage() {
  ...
  const [orderMode, setOrderMode] = useState("market"); // market | limit | stoploss | bracket
  ...
  // Load portfolio + transactions + pending + watchlist
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    pendingOrders.forEach(async (order) => {
      ...
      // Check bracket SL trigger
      if (order.slTrigger && order.orderSide === "buy" && stock.ltp <= order.slTrigger) shouldExecute = true;
      if (order.targetTrigger && order.orderSide === "buy" && stock.ltp >= order.targetTrigger) shouldExecute = true;
  ...
  async function executeTrade() {
    ...
    if (orderMode === "market") {
      ...
      if (orderMode === "bracket") {
        order.slTrigger = parseFloat(slPrice) || 0;
        order.targetTrigger = parseFloat(targetPrice) || 0;
      }
      ...
      setOrderResult({ ok: true, msg: `${labels[orderMode]} order placed: ${orderSide.toUpperCase()} ${qty} ${selectedStock} at ₹${triggerPrice.toFixed(2)}` });
    }
  ...
  const analytics = useMemo(() => {
    ...
    return { totalTrades: sells.length, wins: wins.length, losses: losses.length, winRate: Math.round(wins.length / sells.length * 100), totalPnl, avgGain, avgLoss, best, worst };
  }, [transactions]);
  ...
  const searchResults = search.length >= 1 ? Object.values(prices).filter(s => s.symbol.toLowerCase().includes(search.toLowerCase()) || s.name.toLowerCase().includes(search.toLowerCase())).sort((a, b) => { const aExact = a.symbol.toLowerCase() === search.toLowerCase() ? 0 : 1; const bExact = b.symbol.toLowerCase() === search.toLowerCase() ? 0 : 1; return aExact - bExact || a.symbol.localeCompare(b.symbol); }).slice(0, 8) : [];
  ...
  const pieData = portfolio.holdings.map((h, i) => {
    const val = h.qty * (prices[h.symbol]?.ltp || h.avgPrice);
    return { symbol: h.symbol, value: val, color: PIE_COLORS[i % PIE_COLORS.length] };
  });
    ...
    <div>
      ...
      <div className="trading-layout">
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20 }}>
          ...
          {/* Stock Search — Rich Dropdown */}
          <div style={{ marginBottom: 14, position: "relative" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={S.label}>Stock</div>
              <button onClick={() => toggleWatchlist(selectedStock)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, padding: 0 }} title={isInWatchlist ? "Remove from watchlist" : "Add to watchlist"}>
                {isInWatchlist ? "⭐" : "☆"}
              </button>
            </div>
            <div style={{ position: "relative" }}>
              <input type="text" value={search} placeholder={`${selectedStock} — ${stock.name || "Search..."}`}
              ...
              />
              {stock.ltp > 0 && !search && (
                <div style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", ...S.mono, fontSize: 13, fontWeight: 600, color: stock.change >= 0 ? "var(--green)" : "var(--red)" }}>₹{stock.ltp?.toFixed(2)}</div>
              )}
            </div>
            {showResults && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 12px 40px rgba(0,0,0,0.4)", zIndex: 50, marginTop: 4, maxHeight: 320, overflowY: "auto" }}>
                {/* Watchlist section */}
                {!search && watchlist.length > 0 && (
                  <div>
                    <div style={{ padding: "8px 14px", fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.8, borderBottom: "1px solid var(--border)" }}>⭐ Watchlist</div>
                    {watchlist.map(sym => { const s = prices[sym]; if (!s) return null; return (
                      <div key={sym} onMouseDown={() => { setSelectedStock(sym); setSearch(""); setShowResults(false); setQuantity(""); setOrderResult(null); }}
                        ...
                        <div><div style={{ fontWeight: 700, fontSize: 13 }}>{s.symbol}</div><div style={{ fontSize: 11, color: "var(--text-muted)" }}>{s.name}</div></div>
                        <div style={{ textAlign: "right" }}><div style={{ ...S.mono, fontSize: 13, fontWeight: 600 }}>₹{s.ltp?.toFixed(2)}</div><div style={{ fontSize: 11, fontWeight: 600, color: s.change >= 0 ? "var(--green)" : "var(--red)" }}>{s.change >= 0 ? "+" : ""}{s.change?.toFixed(2)}%</div></div>
                      </div>
                    ); })}
                  </div>
                )}
                ...
                {search && searchResults.length > 0 && (
                  <div>
                    <div style={{ padding: "8px 14px", fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.8, borderBottom: "1px solid var(--border)" }}>Search Results</div>
                    {searchResults.map(s => (
                      <div key={s.symbol} onMouseDown={() => { setSelectedStock(s.symbol); setSearch(""); setShowResults(false); setQuantity(""); setOrderResult(null); }}
                        style={{ padding: "10px 14px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border)" }}
                        onMouseEnter={e => e.currentTarget.style.background = "var(--hover-bg)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <div><div style={{ fontWeight: 700, fontSize: 13 }}>{s.symbol} {watchlist.includes(s.symbol) && <span style={{ fontSize: 10 }}>⭐</span>}</div><div style={{ fontSize: 11, color: "var(--text-muted)", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</div></div>
                        <div style={{ textAlign: "right" }}><div style={{ ...S.mono, fontSize: 13, fontWeight: 600 }}>₹{s.ltp?.toFixed(2)}</div><div style={{ fontSize: 11, fontWeight: 600, color: s.change >= 0 ? "var(--green)" : "var(--red)" }}>{s.change >= 0 ? "+" : ""}{s.change?.toFixed(2)}%</div></div>
                      </div>
                    ))}
                  </div>
                ...
                {!search && watchlist.length === 0 && <div style={{ padding: 16, textAlign: "center", color: "var(--text-muted)", fontSize: 12 }}>Start typing to search stocks, or add stocks to your watchlist with ⭐</div>}
              </div>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 14, background: "var(--bg-secondary)", borderRadius: 8, padding: 3 }}>
            {["buy", "sell"].map(side => (<button key={side} onClick={() => { setOrderSide(side); setOrderResult(null); }} style={{ flex: 1, padding: 8, border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer", background: orderSide === side ? (side === "buy" ? "var(--green-dim)" : "var(--red-dim)") : "transparent", color: orderSide === side ? (side === "buy" ? "var(--green)" : "var(--red)") : "var(--text-secondary)", fontFamily: "'DM Sans',sans-serif", textTransform: "uppercase", letterSpacing: 0.5 }}>{side}</button>))}
          </div>

          {/* Order type */}
          <div style={{ display: "flex", gap: 3, marginBottom: 14 }}>
            {[{ key: "market", label: "Market" }, { key: "limit", label: "Limit" }, { key: "stoploss", label: "SL" }, { key: "bracket", label: "Bracket" }].map(m => (
              <button key={m.key} onClick={() => { setOrderMode(m.key); setOrderResult(null); }} style={{ flex: 1, padding: "6px 2px", borderRadius: 6, fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", background: orderMode === m.key ? "var(--blue-dim)" : "transparent", border: `1px solid ${orderMode === m.key ? "rgba(96,165,250,0.3)" : "var(--border)"}`, color: orderMode === m.key ? "var(--blue)" : "var(--text-muted)" }}>{m.label}</button>
            ))}
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={S.label}>Quantity</div><div style={{ fontSize: 10, color: "var(--text-muted)" }}>Max: <span style={{ ...S.mono, color: "var(--blue)", cursor: "pointer" }} onClick={() => setQuantity(String(maxQty))}>{maxQty}</span></div></div>
            <input type="number" value={quantity} onChange={e => { setQuantity(e.target.value); setOrderResult(null); }} placeholder="Shares" style={S.input} />
            <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
              {QTY_BUTTONS.map(q => (<button key={q} onClick={() => setQuantity(String(q))} style={{ flex: 1, padding: "5px 2px", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "'JetBrains Mono',monospace", background: parseInt(quantity) === q ? "var(--green-dim)" : "var(--surface)", border: `1px solid ${parseInt(quantity) === q ? "var(--green)" : "var(--border)"}`, color: parseInt(quantity) === q ? "var(--green)" : "var(--text-muted)" }}>{q}</button>))}
              <button onClick={() => setQuantity(String(maxQty))} style={{ flex: 1, padding: "5px 2px", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", background: parseInt(quantity) === maxQty && maxQty > 0 ? "var(--blue-dim)" : "var(--surface)", border: `1px solid ${parseInt(quantity) === maxQty && maxQty > 0 ? "var(--blue)" : "var(--border)"}`, color: parseInt(quantity) === maxQty && maxQty > 0 ? "var(--blue)" : "var(--text-muted)" }}>MAX</button>
            </div>
          </div>

          {/* Summary */}
          {confirmQty > 0 && stock.ltp > 0 && (<div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: 12, marginBottom: 14, fontSize: 13 }}><div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span style={{ color: "var(--text-muted)" }}>Price</span><span style={{ fontWeight: 600, ...S.mono }}>{orderMode === "market" ? `₹${stock.ltp?.toFixed(2)}` : `₹${confirmPrice.toFixed(2)} (${orderMode})`}</span></div><div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span style={{ color: "var(--text-muted)" }}>Qty</span><span style={{ fontWeight: 600, ...S.mono }}>{quantity}</span></div>{orderMode === "bracket" && slPrice && <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span style={{ color: "var(--text-muted)" }}>SL</span><span style={{ fontWeight: 600, ...S.mono, color: "var(--red)" }}>₹{parseFloat(slPrice).toFixed(2)}</span></div>}{orderMode === "bracket" && targetPrice && <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span style={{ color: "var(--text-muted)" }}>Target</span><span style={{ fontWeight: 600, ...S.mono, color: "var(--green)" }}>₹{parseFloat(targetPrice).toFixed(2)}</span></div>}<div style={{ height: 1, background: "var(--border)", margin: "5px 0" }} /><div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700 }}><span>Total</span><span style={{ ...S.mono, color: orderSide === "buy" ? "var(--green)" : "var(--red)" }}>₹{confirmTotal.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</span></div>{orderSide === "buy" && confirmTotal <= portfolio.cash && <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4 }}>Remaining: {fmt(portfolio.cash - confirmTotal)}</div>}</div>)}
      ...
      {/* Confirmation Dialog */}
      {showConfirm && (<div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setShowConfirm(false)}><div onClick={e => e.stopPropagation()} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 28, maxWidth: 400, width: "100%" }}><div style={{ fontSize: 18, fontWeight: 700, marginBottom: 16 }}>Confirm Order</div><div style={{ background: "var(--bg-secondary)", borderRadius: 10, padding: 16, marginBottom: 20 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ color: "var(--text-muted)", fontSize: 13 }}>Action</span><span style={{ fontWeight: 700, fontSize: 14, color: orderSide === "buy" ? "var(--green)" : "var(--red)", textTransform: "uppercase" }}>{orderSide} · {orderMode}</span></div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ color: "var(--text-muted)", fontSize: 13 }}>Stock</span><span style={{ fontWeight: 600 }}>{selectedStock}</span></div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ color: "var(--text-muted)", fontSize: 13 }}>Qty</span><span style={{ fontWeight: 600, ...S.mono }}>{quantity}</span></div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ color: "var(--text-muted)", fontSize: 13 }}>Price</span><span style={{ fontWeight: 600, ...S.mono }}>₹{confirmPrice.toFixed(2)}</span></div>{orderMode === "bracket" && <><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ color: "var(--text-muted)", fontSize: 13 }}>SL</span><span style={{ fontWeight: 600, ...S.mono, color: "var(--red)" }}>₹{parseFloat(slPrice || 0).toFixed(2)}</span></div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ color: "var(--text-muted)", fontSize: 13 }}>Target</span><span style={{ fontWeight: 600, ...S.mono, color: "var(--green)" }}>₹{parseFloat(targetPrice || 0).toFixed(2)}</span></div></>}<div style={{ height: 1, background: "var(--border)", margin: "10px 0" }} /><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 700, fontSize: 15 }}>Total</span><span style={{ fontWeight: 700, ...S.mono, fontSize: 18, color: orderSide === "buy" ? "var(--green)" : "var(--red)" }}>₹{confirmTotal.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</span></div></div><div style={{ display: "flex", gap: 10 }}><button onClick={() => setShowConfirm(false)} style={{ flex: 1, padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 600, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-secondary)", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>Cancel</button><button onClick={executeTrade} style={{ flex: 1, padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, border: "none", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", background: orderSide === "buy" ? "var(--gradient-green)" : "linear-gradient(135deg, #f87171, #ef4444)", color: orderSide === "buy" ? "var(--btn-text)" : "#fff" }}>Confirm</button></div></div></div>)}

      {/* Portfolio Allocation + Analytics */}
      {(portfolio.holdings.length > 0 || analytics) && (
        <div style={{ display: "grid", gridTemplateColumns: portfolio.holdings.length > 0 && analytics ? "1fr 1fr" : "1fr", gap: 16, marginTop: 20 }}>
          {/* Pie Chart */}
          {portfolio.holdings.length > 0 && (
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Portfolio Allocation</div>
                ...
                <svg viewBox="0 0 120 120" style={{ width: 120, height: 120, flexShrink: 0 }}>
                  {(() => {
                    ...
                    return pieData.map((d, i) => {
                      ...
                      if (pct >= 0.999) return <circle key={i} cx="60" cy="60" r="50" fill={d.color} />;
                      const s = (startAngle - 90) * Math.PI / 180;
                      const e = (startAngle + angle - 90) * Math.PI / 180;
                      const large = angle > 180 ? 1 : 0;
                      const x1 = 60 + 50 * Math.cos(s), y1 = 60 + 50 * Math.sin(s);
                      const x2 = 60 + 50 * Math.cos(e), y2 = 60 + 50 * Math.sin(e);
                      return <path key={i} d={`M60,60 L${x1},${y1} A50,50 0 ${large},1 ${x2},${y2} Z`} fill={d.color} />;
                    });
                  })()}
                  <circle cx="60" cy="60" r="30" fill="var(--bg-card)" />
                  <text x="60" y="57" textAnchor="middle" fill="var(--text-primary)" fontSize="12" fontWeight="700" fontFamily="'JetBrains Mono',monospace">{portfolio.holdings.length}</text>
                  <text x="60" y="70" textAnchor="middle" fill="var(--text-muted)" fontSize="8">stocks</text>
                </svg>
                <div style={{ flex: 1 }}>
                  {pieData.map((d, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", fontSize: 12 }}>
                      <span style={{ width: 8, height: 8, borderRadius: 2, background: d.color, flexShrink: 0 }} />
                      <span style={{ fontWeight: 600, flex: 1 }}>{d.symbol}</span>
                      <span style={{ ...S.mono, color: "var(--text-muted)" }}>{pieTotal > 0 ? Math.round(d.value / pieTotal * 100) : 0}%</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {analytics && (
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Performance Analytics</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: 10, textAlign: "center" }}><div style={{ ...S.mono, fontSize: 20, fontWeight: 700 }}>{analytics.totalTrades}</div><div style={{ fontSize: 10, color: "var(--text-muted)" }}>Total Trades</div></div>
                <div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: 10, textAlign: "center" }}><div style={{ ...S.mono, fontSize: 20, fontWeight: 700, color: analytics.winRate >= 50 ? "var(--green)" : "var(--red)" }}>{analytics.winRate}%</div><div style={{ fontSize: 10, color: "var(--text-muted)" }}>Win Rate</div></div>
                <div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: 10, textAlign: "center" }}><div style={{ ...S.mono, fontSize: 16, fontWeight: 700, color: "var(--green)" }}>{fmt(analytics.avgGain)}</div><div style={{ fontSize: 10, color: "var(--text-muted)" }}>Avg Winning Trade</div></div>
                <div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: 10, textAlign: "center" }}><div style={{ ...S.mono, fontSize: 16, fontWeight: 700, color: "var(--red)" }}>{fmt(analytics.avgLoss)}</div><div style={{ fontSize: 10, color: "var(--text-muted)" }}>Avg Losing Trade</div></div>
              </div>
              <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                <div style={{ flex: 1, padding: "8px 10px", borderRadius: 6, background: "var(--green-dim)", fontSize: 11 }}><div style={{ fontWeight: 700, color: "var(--green)" }}>Best Trade</div><div style={{ color: "var(--green)", ...S.mono }}>{analytics.best.symbol} +{fmt(analytics.best.pnl)}</div></div>
                <div style={{ flex: 1, padding: "8px 10px", borderRadius: 6, background: "var(--red-dim)", fontSize: 11 }}><div style={{ fontWeight: 700, color: "var(--red)" }}>Worst Trade</div><div style={{ color: "var(--red)", ...S.mono }}>{analytics.worst.symbol} {fmt(analytics.worst.pnl)}</div></div>
              </div>
            </div>
          )}
        </div>
